...
if (! function_exists('getUserInfo')) {
    ...
    if(Auth::user()){
        ...
        $user_detail = Profile::where([
            'user_id' => $data['userId'] ?? null,
            'role_id' => $data['roleId'] ?? null
        ])
        ->select('image','first_name','last_name','slug')
        ->first();

        // Handle user image
        if($user_detail && !empty($user_detail->image)){
            $file_url = getProfileImageURL($user_detail->image, $image_dimension);
            $user_image = !empty($file_url) 
                ? 'storage/' . $file_url 
                : 'images/default-user-'.$image_dimension.'.png';
        } else {
            $user_image = 'images/default-user-'.$image_dimension.'.png';
        }

        // Prevent crash when user_detail is NULL
        $info = [
            'role_id'       => $data['roleId'] ?? null,
            'user_role'     => $data['roleName'] ?? null,
            'user_name'     => $user_detail 
                                ? $user_detail->first_name.' '.$user_detail->last_name 
                                : '',
            'user_image'    => $user_image,
            'slug'          => $user_detail->slug ?? null,
        ];
    }
