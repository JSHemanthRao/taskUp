...
// Temporary Test Route (Top Priority)
Route::get('/test-mail', function () {
    try {
        \Illuminate\Support\Facades\Mail::raw('Test email from TaskUp', function ($message) {
            $message->to('test@example.com')
                ->subject('Test Email');
        });
        return 'Email sent successfully via ' . config('mail.default') . ' using host ' . config('mail.mailers.smtp.host');
    } catch (\Exception $e) {
